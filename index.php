...
<body>
    <header>
        <h2>📅 Agendamento de Aulas - SENAI União da Vitória</h2>
        <div class="cadastra">
            <a href="api/cadastro_professor.php">Cadastrar professor</a>
            <a href="api/cadastro_aula.php">Cadastrar aulas</a>
            <a href="api/cadastro_uc.php">Cadastrar UC</a>
        </div>
    </header>
    ...
    <script>
        ...
        // ✂️ Gera versão abreviada do nome da UC
        function abreviarUC(nome) {
            if (!nome) return '';
                    ...
                    ['Manhã', 'Tarde', 'Noite'].forEach(turno => {
                        ...
                        diasComDatas.forEach(d => {
                            ...
                            if (aula && aula.descricao) {
                                const ucAbreviada = abreviarUC(aula.uc_nome);

                                td.innerHTML = `
                                    <div class="aula" style="background:${aula.cor};">
                                        <div class="conteudo">
                                            <div class="sigla">${aula.sigla || ''}</div>
                                            ${aula.uc_nome ? `<div class="uc-badge">${aula.uc_nome.split(' ')[0]}</div>` : ''}
                                        </div>
                                        <div class="acoes">
        ...
        function abrirModal(professor, turno, dia) {
            ...
            // 🔹 Carrega as aulas (cursos) do professor
            selectProf.dispatchEvent(new Event('change'));

            // 🔹 Aguarda o carregamento das aulas e dispara o carregamento das UCs
            setTimeout(() => {
                const selectCurso = document.getElementById('aula_select');
        ...
        // 🧩 Atualizar cursos (tipos de aula) ao trocar o professor
        const selectProfessor = document.getElementById('professor_select');
        const selectCurso = document.getElementById('aula_select');

        selectProfessor.addEventListener('change', async () => {
            const id = selectProfessor.value;
            selectCurso.innerHTML = '<option>Carregando...</option>';
            ...
            if (aulas.length > 0) {
                selectCurso.innerHTML =
                    '<option value="">Selecione o curso</option>' +
                    aulas.map(a => `<option value="${a.id}">${a.sigla} - ${a.descricao}</option>`).join('');
            } else {
                selectCurso.innerHTML = '<option>Nenhum curso cadastrado</option>';
            }

            // Limpa as UCs anteriores
            document.getElementById('uc_select').innerHTML = '<option value="">Selecione a UC</option>';
        });

        // 🧩 Atualizar UCs ao trocar o curso
        const selectUC = document.getElementById('uc_select');

        selectCurso.addEventListener('change', async () => {
            const tipoAulaId = selectCurso.value;
            selectUC.innerHTML = '<option>Carregando...</option>';

            const res = await fetch(`api/listar_uc.php?tipo_aula_id=${tipoAulaId}`);
            const ucs = await res.json();

            if (ucs.length > 0) {
                selectUC.innerHTML =
                    '<option value="">Selecione a UC</option>' +
                    ucs.map(uc => `<option value="${uc.id}">${uc.nome}</option>`).join('');
            } else {
                selectUC.innerHTML = '<option>Nenhuma UC cadastrada</option>';
            }
        });
        ...
        document.getElementById('formAula').addEventListener('submit', async (e) => {
            ...
            const dados = {
                ...
                uc_id: document.getElementById('uc_select').value, // 🧩 adiciona aqui
        ...
        function editarAula(id) {
            fetch(`api/obter_aula.php?id=${id}`)
                ...
                .then(aula => {
                    ...
                    // 🔹 Define professor
                    selectProf.value = aula.professor_id;
                    selectProf.dispatchEvent(new Event('change')); // carrega cursos

                    // 🔹 Aguarda cursos carregarem e define o curso
                    setTimeout(() => {
                        selectCurso.value = aula.tipo_aula_id;
                        selectCurso.dispatchEvent(new Event('change')); // carrega UCs

                        // 🔹 Aguarda UCs carregarem e define a UC
                        setTimeout(() => {
                            if (aula.uc_id) selectUC.value = aula.uc_id;
                        }, 300);
                    }, 400);
